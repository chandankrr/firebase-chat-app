...
const ChatFooter = () => {
  ...
  return (
    <div className="flex items-center bg-c1/[0.5] p-2 rounded-xl relative">
      ...
      {isTyping && (
        <div className="absolute -top-6 left-4 bg-c2 w-full h-6">
          <div className="flex gap-2 w-full h-full opacity-50 text-sm text-white">
            ...
            <Image src="/typing.svg" width={24} height={24} alt="typing img" />
          </div>
        </div>
      )}
      ...
      {editMsg && (
        <div
          className="absolute -top-12 left-1/2 -translate-x-1/2 bg-c4 flex items-center gap-2 py-2 px-4 pr2 rounded-full text-sm font-semibold cursor-pointer shadow-lg"
          onClick={() => setEditMsg(null)}
        >
          <span>Cancle edit</span>
